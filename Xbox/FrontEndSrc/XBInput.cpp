...
// Time step = 1/60th of a sec. This should not be hacked in this way.
...
HRESULT XBInput_InitControllers()
{
    ...
    for( DWORD i = 0; i < XGetPortCount(); i++ )
    {
        ...
        if( dwDeviceMask & (1<<i) ) 
        {
            ...
            // Reject unsupported device types
            if( XBINPUT_CONTROLLER_UNSUPPORTED == g_Controllers[i].dwDeviceType )
...
HRESULT XBInput_ReadControllers()
{
    ...
    for( DWORD i=0; i < XGetPortCount(); i++ )
    {
		// Conveneient access to the i'th controller
...
VOID XBInput_GetInput()
{
    ...
    for( DWORD i = 0; i < 4; i++ )
    {
        ...
        // Use sterring wheel to mimic DPad
