...
//$MODIFIED
//    sprintf(buf, "Draw Device: %s", DrawDevices[CurrentDrawDevice].Name);
    sprintf(buf, "Draw Device: %s", "Xbox draw device");
//$END_MODIFICATIONS
    DrawGameText(16, 16, GAME_GAUGE_RGB, buf);

    sprintf(buf, "Resolution: %ld x %ld x %ld", ScreenXsize, ScreenYsize, ScreenBpp);
    DrawGameText(16, 40, GAME_GAUGE_RGB, buf);

    sprintf(buf, "Textures: %s", RegistrySettings.Texture24 ? "24 Bit" : "16 Bit");
    DrawGameText(16, 64, GAME_GAUGE_RGB, buf);

    DrawGameText(16, 88, GAME_GAUGE_RGB, BackBufferCount == 1 ? "Double Buffered" : "Triple Buffered");

    sprintf(buf, "Time: %.2f", (float)TIME2MS(CurrentTimer() - TOTAL_START_TIME) / 1000.0f);
    DrawGameText(16, 112, GAME_GAUGE_RGB, buf);

    sprintf(buf, "Frames: %ld", FRAMES_TOTAL);
    DrawGameText(16, 134, GAME_GAUGE_RGB, buf);

    sprintf(buf, "Cur FPS: %ld", CUR_ONE_SECOND);
    DrawGameText(16, 158, GAME_GAUGE_RGB, buf);

    if (MIN_ONE_SECOND != 0xffffffff)
    {
        sprintf(buf, "Min FPS: %ld", MIN_ONE_SECOND);
        DrawGameText(16, 182, GAME_GAUGE_RGB, buf);
    }

    if (MAX_ONE_SECOND)
    {
        sprintf(buf, "Max FPS: %ld", MAX_ONE_SECOND);
        DrawGameText(16, 206, GAME_GAUGE_RGB, buf);
    }
}
