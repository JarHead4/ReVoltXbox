...
static bool IsUserLevel(char *dir);
...
bool LEV_InitLevelStageOne()
{
    ...
    LoadCurrentLevelInfo(levelInfo->Dir);
...
// look up texture resources in world resource bundle.
  if (World.m_pXBR != NULL)
  {
    ...
    for (ii = 0 ; ii < TPAGE_WORLD_NUM ; ii++)
    {
        ...
        if( -1 == GetFileAttributes(buf) )  { continue; }  // make sure file exists
        //$PERF: checking per-file existence here is stupid.  We should fix this.
...
//$END_MODIFICATIONS
    ...
    {
//$MODIFIED
//        wsprintf(buf, "levels\\%s\\%s%c.bmp", CurrentLevelInfo.Dir, CurrentLevelInfo.Dir, ii + 'a');
        wsprintf(buf, "D:\\levels\\%s\\%s%c.bmp", CurrentLevelInfo.Dir, CurrentLevelInfo.Dir, ii + 'a');
//$END_MODIFICATIONS
...
//$REMOVED            if ((stricmp(buf, "SHarrison") == 0) || (stricmp(buf, "PPhippen") == 0))
    ...
    //$ADDITION
    // Load sfx
    g_SoundEngine.LoadLevel(CurrentLevelInfo.Dir);
    //$END_ADDITION
#endif // !OLD_AUDIO
...
#ifdef OLD_AUDIO //$NOTE: REPLACEMENT CODE IS IN StageOne
    // load sfx
...
//$ADDITION(jedl) - export world if not yet bundled
    #pragma message( "Hey Jed: is this the right place to export the world?  Could do in StageOne, but static level models won't be loaded yet..." )
...
void UnloadTrackDataStageTwo()
{
    ...
    StopRedbook();
#else
    #pragma message( "JHarding needs to finish porting this." )
    // stop music
    g_MusicManager.Stop();
#endif // OLD_AUDIO
...
void LoadingDisplay(char *text)
{
    short pos;

// dump text on both buffers

    pos = (640 - (strlen(text) * 12)) / 2;
    ...
    InitRenderStates();
    BeginTextState();
    DrawWideGameText(pos, LoadingPos, 0xffffff, text);
    D3Ddevice->EndScene();
