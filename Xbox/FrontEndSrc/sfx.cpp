...
#else //!OLD_AUDIO
    #pragma message( "JHarding needs to port (change code that uses this function)." )
    return FALSE;
#endif //!OLD_AUDIO
}
...
//        if (!SfxLoad[i].Pos)
//        {
//            wsprintf(buf, "Can't load generic sfx '%s' into slot %d!", wavs[i], i);
...
//            if (!SfxLoad[i].Pos)
//            {
//                wsprintf(buf, "Can't load level sfx '%s' into slot %d!", wavs[i - SfxLoadNum], i);
//                DumpMessage(NULL, buf);
//            }
//            else
//            {
...
//$TODO: not handling per-level sounds yet!
...
//    return NULL;
    ...
    if( num < SFX_GENERIC_NUM ) //$BUGBUG: we only support the non-level-specific sounds right now
    {
        CSoundEffectInstance* pInstance;
        g_SoundEngine.Play2DSound( num, FALSE, &pInstance );
        return (SAMPLE_SFX*)(pInstance);
    }
    return NULL;  //$NOTE: this is probably okay; functions *almost* never use retval from PlaySfx/PlaySfx3D (with notable exception of piano handler)
//$END_MODIFICATIONS
...
//    AIL_end_sample(sample->Handle);
    ...
    g_SoundEngine.ReturnInstance( (CSoundEffectInstance*)(sample) );  //$TODO: is this correct for StopSfx() ?
//$END_MODIFICATIONS
...
//    sample3d->Alive = FALSE;
    ...
    g_SoundEngine.ReturnInstance( (CSoundEffectInstance*)(sample3d) );
//$END_MODIFICATIONS
...
//    sample3d->Num = sfx;
    ...
    //g_SoundEngine.ReturnInstance( (CSoundEffectInstance*)(sample3d) );  //$TODO: is this right for ChangeSfxSample3d() ??
//$END_MODIFICATIONS
