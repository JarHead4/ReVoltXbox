...
void SendWeaponNew(VEC *pos, MAT *mat, unsigned long id, long weapon, LARGE_INTEGER GlobalID)
{
    WEAPON_REMOTE_DATA *data = (WEAPON_REMOTE_DATA*)(SendHeader + 1);
    QUATERNION quat;

// setup header

    SendHeader->Type = MESSAGE_WEAPON_DATA;

// set pos

    CopyVec(pos, &data->Pos);

// set quat

    MatToQuat(mat, &quat);
    data->QuatX = (char)(quat.v[VX] * REMOTE_QUAT_SCALE);
    data->QuatY = (char)(quat.v[VY] * REMOTE_QUAT_SCALE);
    data->QuatZ = (char)(quat.v[VZ] * REMOTE_QUAT_SCALE);
    data->QuatW = (char)(quat.v[S] * REMOTE_QUAT_SCALE);
...
// send
    ...
    QueueMessage(SendHeader, sizeof(MESSAGE_HEADER) + sizeof(WEAPON_REMOTE_DATA));
}
...
void ProcessWeaponNew()
{
    long flag[2];
    float lag;
    void *w;
    PLAYER *player;
    WEAPON_REMOTE_DATA *data = (WEAPON_REMOTE_DATA*)(ReceiveHeader + 1);
    OBJECT *obj;
    VEC pos;
    MAT mat;
    QUATERNION quat;
...
// get player
    ...
    player = GetPlayerFromPlayerID(data->ID);
    if (!player)
        return;
...
// reset player pickup
    ...
    player->PickupType = PICKUP_TYPE_NONE;
    player->PickupNum = 0;

// get pos

    CopyVec(&data->Pos, &pos);

// get mat

    quat.v[VX] = (REAL)data->QuatX / REMOTE_QUAT_SCALE;
    quat.v[VY] = (REAL)data->QuatY / REMOTE_QUAT_SCALE;
    quat.v[VZ] = (REAL)data->QuatZ / REMOTE_QUAT_SCALE;
    quat.v[S] = (REAL)data->QuatW / REMOTE_QUAT_SCALE;
    NormalizeQuat(&quat);
    QuatToMat(&quat, &mat);

// create weapon

    if (data->Weapon == OBJECT_TYPE_OILSLICK_DROPPER || data->Weapon == OBJECT_TYPE_PICKUP)
    {
        obj = CreateObject(&pos, &mat, data->Weapon, flag);
    }
    else
    {
        obj = CreateObject(&player->car.Body->Centre.Pos, &player->car.Body->Centre.WMatrix, data->Weapon, flag);
    }
...
// adjust for lag
    ...
    lag = (float)(TotalRaceTime - data->Time) / 1000.0f;
    w = obj->Data;
...
{
    SendHeader->Type = MESSAGE_TARGET_STATUS_DATA;
    TARGET_STATUS_DATA *data = (TARGET_STATUS_DATA*)(SendHeader + 1);

    data->ID = id;
    data->Status = status;

    QueueMessage(SendHeader, sizeof(MESSAGE_HEADER) + sizeof(TARGET_STATUS_DATA));
}
...
void ProcessTargetStatus(void)
{
    TARGET_STATUS_DATA *data = (TARGET_STATUS_DATA*)(ReceiveHeader + 1);
    PLAYER *player;
...
// sfx
 ...
 #else // !OLD_AUDIO
 #pragma message( "Please port me, Mr. Harding" )
 #endif // !OLD_AUDIO
#endif
