    ...
    case CALCSTATS_STATE_RACING:
        ...
        if (!CalcStatsVars.GotAcc) {
            if (vel > MPH2OGU_SPEED * 25) {
                ...
                DrawGameText(100, 50, 0xff0000, "Calculating Acc");
            }
        }
        ...
        if (!CalcStatsVars.GotSpeed) {
            if (CalcStatsVars.GotAcc && (acc < TO_ACC(Real(0.1)))) {
                ...
                DrawGameText(100, 70, 0xff0000, "Calculating Speed");
            }
        }
        ...
        if (CalcStatsVars.GotAcc && CalcStatsVars.GotSpeed) {
            ...
            DrawMenuText(50, 120, 0xffffffff, MenuBuffer);
            sprintf(MenuBuffer, "Acc: %d", Int(FROM_TIME(CarInfo[CalcStatsVars.CarType].Acc)*100));
            DrawMenuText(50, 140, 0xffffffff, MenuBuffer);
#endif
        }
        break;
...
// update engine
    ...
    if (car->SfxEngine)
    {
#pragma message( "CPrince: put this back!? (OLD_AUDIO test)" )
/**CPRINCE TESTING: TEMP REMOVAL
...
**/        
...
#else // !OLD_AUDIO
    #pragma message( "Review me, Mr. Harding" )
  ...
  /**
    // TODO (JHarding): This should be encapsulated by the instance,
    // no game code should have to go to buffers
    car->pEngineSound->m_apBuffers[0]->SetFrequency( 6000 + 120 * revs );
    // Set engine volume based off revs
    car->pEngineSound->m_apBuffers[0]->SetVolume( -1000 + revs / 2 );
  **/
#endif // !OLD_AUDIO
...
#else // !OLD_AUDIO
    #pragma message( "JHarding needs to change this.  Game quickly crashes when this isn't disabled." )
    /**
        // Changing material...
        if( car->ScrapeMaterial != car->Body->ScrapeMaterial )
        {
            ...
            // Start the new sound
            g_SoundEngine.PlaySubmixedSound( EFFECT_SkidNormal, 
                                             TRUE, 
                                             car->pSourceMix,
                                             &car->pScrapeSound );
        }
...
**/            
    ...
    {
#pragma message( "JHarding needs to port this." )
    }
...
#pragma message( "JHarding needs to port pServoSound, I think." )
...
#else // !OLD_AUDIO
        #pragma message( "Review me, Mr. Harding" )
        g_SoundEngine.Play3DSound( EFFECT_BallDrop, 
                                   FALSE, 
                                   car->Body->Centre.Pos.v[0], 
                                   car->Body->Centre.Pos.v[1], 
                                   car->Body->Centre.Pos.v[2] );
...
#else // !OLD_AUDIO
    #pragma message( "Review me, Mr. Harding" )
            // This will drive you crazy right now
            g_SoundEngine.Play3DSound( EFFECT_Clockwork, FALSE, obj );
#endif // !OLD_AUDIO
            #endif
        }
    }
...
// whistle?
  ...
  #else // !OLD_AUDIO
            #pragma message( "Review me, Mr. Harding" )
            g_SoundEngine.Play3DSound( EFFECT_StarFire, FALSE, vec.v[0], vec.v[1], vec.v[2] );
  #endif // !OLD_AUDIO
#endif
        }
    }
...
#else // !OLD_AUDIO
  #pragma message( "JHarding needs to fix this quick-and-dirty CPrince port." )
...
#ifdef _PC
                if (GameSettings.GameType == GAMETYPE_MULTI && player->type == PLAYER_REMOTE)
                    continue;
#endif

// skip if race and player already has a pickup

                if ((player->PickupNum || player->PickupCycleSpeed) && (GameSettings.GameType == GAMETYPE_SINGLE || GameSettings.GameType == GAMETYPE_CLOCKWORK || GameSettings.GameType == GAMETYPE_MULTI || GameSettings.GameType == GAMETYPE_CHAMPIONSHIP))
                    continue;
...
// got one
                ...
                if (BBTestXZY(&obj->body.CollSkin.BBox, &player->car.BBox)) 
                {
...
#ifndef _N64                    
                    if (GameSettings.GameType == GAMETYPE_BATTLE && player == PLR_LocalPlayer)
#else
                    ...
                    else if (GameSettings.GameType == GAMETYPE_SINGLE || GameSettings.GameType == GAMETYPE_CLOCKWORK || GameSettings.GameType == GAMETYPE_MULTI || GameSettings.GameType == GAMETYPE_CHAMPIONSHIP)
                    {
...
{
#pragma message( "Maybe Jed should port this." )
#ifndef XBOX_NOT_YET_IMPLEMENTED
...
#pragma message( "CPrince: is this the correct value to init var with?" )
        //$NOTE(cprince): Maybe they never used this func?  Or value not critical??
