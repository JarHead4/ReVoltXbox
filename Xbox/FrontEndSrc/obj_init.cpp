...
OBJECT_INIT_DATA ObjInitData[] = {
    ...
    NULL, 0, // InitNameWheel, sizeof(NAMEWHEEL_OBJ),
    InitSprinkler, sizeof(SPRINKLER_OBJ),
    InitSprinklerHose, sizeof(SPRINKLER_HOSE_OBJ),
    InitObjectThrower, sizeof(OBJECT_THROWER_OBJ),
    InitBasketBall, 0,
    NULL, 0, // InitTrackScreen, sizeof(OBJECT_TRACKSCREEN_OBJ),
    NULL, 0, // InitClock, sizeof(OBJECT_CLOCK_OBJ),
    NULL, 0, // InitCarBox, sizeof(OBJECT_CARBOX_OBJ),
    InitStream, 0,
    ...
    InitFox, 0,
    InitTumbleweed, 0,
    NULL, 0, // InitSmallScreen, sizeof(OBJECT_SMALLSCREEN_OBJ),
...
// pick random star if battle, or all if not
    ...
    star = GameSettings.GameType == GAMETYPE_BATTLE ? rand() % TotalStarNum : -1;
...
// zero total pickup / star count
    ...
    /*
    if (g_TitleScreenData.iLevelNum == LEVEL_NEIGHBOURHOOD1)
        return;
    */
...
/////////////////////////////////////
LARGE_INTEGER GetGlobalID(DWORD id)
{
    LARGE_INTEGER gid;

    gid.LowPart = GlobalID++;
    gid.HighPart = id;
    return gid;
}
...
// create 3D sfx
 ...
 #else // !OLD_AUDIO
    g_SoundEngine.Play3DSound( EFFECT_Servo, TRUE, obj, &obj->pSfxInstance );
...
// create 3D sfx
 ...
 #else // !OLD_AUDIO
    g_SoundEngine.Play3DSound( EFFECT_Petrol, TRUE, obj, &obj->pSfxInstance );
...
#else
    //$ADDITION
    obj->player->car.pEngineSound  = NULL;
    obj->player->car.pScrapeSound  = NULL;
    obj->player->car.pScreechSound = NULL;
    obj->player->car.pSourceMix    = NULL;
...
// create sfx
 ...
 #else // !OLD_AUDIO
    g_SoundEngine.Play3DSound( EFFECT_Clockwork, TRUE, obj, &obj->pSfxInstance );
...
// quit if wrong type for this game
    ...
    if (game != GAMETYPE_PRACTICE && game != GAMETYPE_TRAINING && game != GAMETYPE_BATTLE && (flags[0] || !GameSettings.AllowPickups))
        return FALSE;
...
#ifdef _PC
        if (GameSettings.GameType == GAMETYPE_MULTI || GameSettings.GameType == GAMETYPE_BATTLE)
        {
...
static long InitRipple(OBJECT *obj, long *flags)
{
#pragma message( "Still need to port procedural texture stuff.  I nominamte Jed. :-b" )
