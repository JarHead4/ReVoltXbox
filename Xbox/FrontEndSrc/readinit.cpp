...
//$MODIFIED(cprince) - insert "D:\" at the start, if it's not already there and filename is non-null
//                    strncpy(CarInfo[CarList[iCar]].ModelFile[ModelList[iModel]], word, MAX_CAR_FILENAME);
                    if( 0 == strlen(word) || 0 == _strnicmp("D:\\", word, 3) ) {
                        strncpy( CarInfo[CarList[iCar]].ModelFile[ModelList[iModel]], word, MAX_CAR_FILENAME );
                    } else {
                        _snprintf( CarInfo[CarList[iCar]].ModelFile[ModelList[iModel]], MAX_CAR_FILENAME, "D:\\%s", word );
...
//                strncpy(CarInfo[CarList[iCar]].TPageFile, word, MAX_CAR_FILENAME);
                if( 0 == strlen(word) || 0 == _strnicmp("D:\\", word, 3) ) {
                    ...
                    _snprintf( CarInfo[CarList[iCar]].TPageFile, MAX_CAR_FILENAME, "D:\\%s", word );
...
//                strncpy(CarInfo[CarList[iCar]].CollFile, word, MAX_CAR_FILENAME);
                if( 0 == strlen(word) || 0 == _strnicmp("D:\\", word, 3) ) {
                    ...
                    _snprintf( CarInfo[CarList[iCar]].CollFile, MAX_CAR_FILENAME, "D:\\%s", word );
...
//$END_MODIFICATIONS
        ...
        // SELECTABLE
            ...
            ReadBool(&tBool, fp);
            for (iCar = 0; iCar < CarListSize; iCar++) {
                CarInfo[CarList[iCar]].Selectable = FALSE;
            }
        }
...
bool ReadAllCarInfoMultiple()
{
    ...
    while (TRUE)
    {
        ...
        for (iCar = 0; iCar < CARID_NTYPES; iCar++) {
            if (_strnicmp(&CarDirs[iCar][5], data.cFileName, strlen(CarDirs[iCar])) == 0) { //$MODIFIED: changed strnicmp to _strnicmp
                newCar = FALSE;
                fclose(fp);
                break;
            }
