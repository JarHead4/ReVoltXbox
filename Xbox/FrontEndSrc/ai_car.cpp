...
// Choose route
    ...
    {
        // Choose route
        pPlayer->CarAI.iRouteCurNode = CAI_ChooseNodeRoute(pPlayer, pPlayer->CarAI.pCurNode);
...
// Update racing/overtaking line bias
    if (pNode)
    {
        if (pPlayer->CarAI.bOvertake != pPlayer->CarAI.bOvertakeLast)
        {
            ...
            {
                if (g_TitleScreenData.CupType == RACE_CLASS_BRONZE)
                {
#ifndef _PSX
...
//          range = DivScalar(-pPlayer->CarAI.futureCurNodeEdgeDist[0], gGazzaWWW);
            ...
            if (x > CTRL_RANGE_MAX)
            {
                ...
                if (x > CTRL_RANGE_MAX)
                    ...
                    pPlayer->controls.dy = x;
            }
            else
                pPlayer->controls.dx = x;
