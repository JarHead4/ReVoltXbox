...
{
    World.m_pXBR = NULL;
    World.m_pEffect = NULL;
    World.m_pEffectDefaultMaterial = NULL;
    ...
    World.m_pEffect = World.m_pXBR->GetEffect( name );
    if (World.m_pEffect == NULL)
        return E_FAIL;
    World.m_pEffectDefaultMaterial = World.m_pXBR->GetEffect( "DefaultMaterial" );
    return S_OK;
}
//$END_ADDITION
...
{
    // Set default state.
    if (World.m_pEffectDefaultMaterial != NULL)
        World.m_pEffectDefaultMaterial->DrawEffect();

   // $TODO: put all of this into the default material
    D3Ddevice->SetRenderState(D3DRS_LIGHTING, FALSE);
    static D3DZBUFFERTYPE zb = D3DZB_TRUE;
    D3Ddevice->SetRenderState(D3DRS_ZENABLE, zb);
    ...
    static DWORD value = FALSE;
    static DWORD ref = 0x08;
    D3Ddevice->SetRenderState(D3DRS_ALPHATESTENABLE, value);
    D3Ddevice->SetRenderState(D3DRS_ALPHAREF, ref);
    static D3DTEXTUREALPHAKILL dwALPHAKILL = D3DTALPHAKILL_ENABLE;
    D3Ddevice->SetTextureStageState(0, D3DTSS_ALPHAKILL, dwALPHAKILL); 
    static D3DTEXTURECOLORKEYOP colorkeyop = D3DTCOLORKEYOP_DISABLE;
    static D3DCOLOR colorkeycolor = 0xff000000;
    D3Ddevice->SetTextureStageState(0, D3DTSS_COLORKEYOP,    colorkeyop);
    D3Ddevice->SetTextureStageState(0, D3DTSS_COLORKEYCOLOR, colorkeycolor);
    D3Ddevice->SetRenderState(D3DRS_TEXTUREFACTOR, 0);

    // Draw the world
    World.m_pEffect->DrawEffect();

    // Restore state
    // TODO: get rid of this.
    D3Ddevice->SetVertexShaderInput(0, 0, 0);
}
//$END_ADDITION
