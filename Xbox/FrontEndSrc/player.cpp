...
void PLR_SetPlayerType(PLAYER *newplayer, PLAYER_TYPE type)
{
    ...
    case PLAYER_GHOST:
        newplayer->car.RenderFlag = CAR_RENDER_GHOST;
...
// set console message
    ...
    wsprintf(buf1, "%d%s: %s: ", i + 1, TEXT_TABLE(i < 3 ? TEXT_ST_NUMABBREV+i : TEXT_ST_NUMABBREV+3), player->PlayerName);
    wsprintf(buf2, "%02d:%02d:%03d", MINUTES(time), SECONDS(time), THOUSANDTHS(time));
    SetConsoleMessage(buf1, buf2, MultiPlayerColours[player->Slot % MAX_RACE_CARS], 0xffffff, 1, CONSOLE_MESSAGE_DEFAULT_TIME);
#endif
#ifdef _N64
    {
    char buf1[32], buf2[32];
    sprintf(buf1, "%d%s: %s", i + 1, TEXT_TABLE(i < 3 ? TEXT_ST_NUMABBREV+i : TEXT_ST_NUMABBREV+3), player->PlayerName);
    sprintf(buf2, "%02d:%02d:%03d", MINUTES(time), SECONDS(time), THOUSANDTHS(time));
    SetConsoleMessage(buf1, buf2, MultiPlayerColours[player->Slot % MAX_RACE_CARS], 0xffffff, 1, Real(4.0));
    }
#endif
