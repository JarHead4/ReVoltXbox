...
//-----------------------------------------------------------------------------
// Name: Initialize()
// Desc: Sets all internal object ptrs to NULL
//-----------------------------------------------------------------------------
VOID CD3DFramework::Initialize()
{
     m_hWnd             = NULL;
...
HRESULT CD3DFramework::Initialize( HWND hWnd, GUID* pDriverGUID, 
                                   GUID* pDeviceGUID,
                                   DDSURFACEDESC2* pddsdMode, DWORD dwFlags )
{
    // Release all the DX objects
    if( FAILED( DestroyObjects() ) )
        return D3DFWERR_NONZEROREFCOUNT;

    // Create the D3D rendering environment (surfaces, device, viewport, etc.)
    HRESULT hr = CreateEnvironment( hWnd, pDriverGUID, pDeviceGUID, 
                                    pddsdMode, dwFlags );
    if( FAILED( hr ) )
    {
        DestroyObjects();
...
//-----------------------------------------------------------------------------
// Name: ChangeDriver()
// Desc: Re-creates the internal objects for the framework
//-----------------------------------------------------------------------------
HRESULT CD3DFramework::ChangeDriver( HWND hWnd, GUID* pDriverGUID,
                                     GUID* pDeviceGUID,
                                     DDSURFACEDESC2* pddsdMode, DWORD dwFlags )
{
    // Release all the DX objects
...
// Desc: Creates the internal objects for the framework
//-----------------------------------------------------------------------------
HRESULT CD3DFramework::CreateEnvironment( HWND hWnd, GUID* pDriverGUID, 
                                          GUID* pDeviceGUID,
                                          ...
                                          DWORD dwFlags )
{
    ...
    // Check params. A NULL device is valid if no 3D objects are desired. A
    // NULL mode is valid if fullscreen operation is not desrired.
    if( NULL==hWnd )
        return E_INVALIDARG;
    if( NULL==pDeviceGUID && (dwFlags&(D3DFW_ZBUFFER|D3DFW_STENCILBUFFER)) )
        return E_INVALIDARG;
    if( NULL==pddsdMode && (dwFlags&D3DFW_FULLSCREEN) )
        return E_INVALIDARG;

    // Initialize all the object ptrs
    Initialize();
    m_hWnd = hWnd;

    // Setup flags if windowed mode is requested
    m_bIsFullscreen = ( dwFlags & D3DFW_FULLSCREEN ) ? TRUE : FALSE;
    
    // Create the DDraw and Direct3D objects
    if( FAILED( hr = CreateDDAndD3D( pDriverGUID, pDeviceGUID, dwFlags ) ) )
        return hr;

    // Create the front and back buffers, and attach a clipper
    if( FAILED( hr = CreateBuffers( pddsdMode, dwFlags ) ) )
        return hr;

    // If the app is not requesting 3D, then this function is done
    if( NULL==pDeviceGUID )
        return S_OK;
...
//-----------------------------------------------------------------------------
// Name: CreateDDAndD3D()
// Desc: Create the DirectDraw and Direct3D pointers
//-----------------------------------------------------------------------------
HRESULT CD3DFramework::CreateDDAndD3D( GUID* pDriverGUID, GUID* pDeviceGUID,
                                       DWORD dwFlags )
{
    LPDIRECTDRAW         pDD;
    ...
    // Unless the user wants otherwise, we set the DDSCL_FPUSETUP flag, which
    // allows D3D to optimize floating point calculations.
    ...
    // 2D apps will not specify a device GUID, since they don't do any 3D.
    if( NULL == pDeviceGUID )
        return S_OK;

    // Next step is to query DirectDraw for access to D3D
    ...
    // Finally, use the FindDevice() method to test if the requested device
    // exists, and if so, what memory type it takes.
    ...
    {
        m_dwDefaultMemType = DDSCAPS_SYSTEMMEMORY;
        memcpy( &m_ddDeviceDesc, &devResult.ddSwDesc, sizeof(D3DDEVICEDESC) );
    }
    else
    {
        m_dwDefaultMemType = DDSCAPS_VIDEOMEMORY;
...
HRESULT CD3DFramework::CreateBuffers( DDSURFACEDESC2* pddsd, DWORD dwFlags )
{
    ...
    // For fullscreen modes, Set the display mode to the requested dimensions
    if( m_bIsFullscreen )
    {
        // Check for 320x200x8 modes, and set flag to avoid using ModeX
        DWORD dwModeFlags = 0;
    ...
    ddsd.ddsCaps.dwCaps = DDSCAPS_PRIMARYSURFACE|m_dwDefaultMemType;
    ...
    if( FALSE == m_bIsFullscreen )
    {
        ...
        if( SUCCEEDED( hr = m_pDD->CreateClipper( 0, &pcClipper, NULL ) ) )
        {
            // Assoicate the clipper with the window
            if( SUCCEEDED( hr = pcClipper->SetHWnd( 0, m_hWnd ) ) )
                // Attach clipper to the front buffer
                hr = m_pddsFrontBuffer->SetClipper( pcClipper );
            
            SAFE_RELEASE( pcClipper ); // Clipper ref is not needed anymore
        }
    ...
    if( dwFlags & D3DFW_BACKBUFFER )
    {
        if( m_bIsFullscreen )
        {
            ...
            hr = m_pddsFrontBuffer->GetAttachedSurface( &ddscaps, 
                                                        &m_pddsBackBuffer );
        }
        else // For windowed-mode rendering
        {
        ...
        if( FAILED( hr ) )
        {
            DEBUG_MSG( TEXT("Error: Can't get the backbuffer") );
...
HRESULT CD3DFramework::CreateZBuffer()
{
    ...
    D3DUtil_InitSurfaceDesc( ddsd, DDSD_WIDTH | DDSD_HEIGHT | DDSD_CAPS |
                             DDSD_PIXELFORMAT );
    ddsd.ddsCaps.dwCaps = DDSCAPS_ZBUFFER | m_dwDefaultMemType;
    ddsd.dwWidth        = m_dwRenderWidth;
    ddsd.dwHeight       = m_dwRenderHeight;
    memcpy( &ddsd.ddpfPixelFormat, &m_ddpfZBuffer, sizeof(DDPIXELFORMAT) );

    // Create and attach a z-buffer
    if( FAILED( m_pDD->CreateSurface( &ddsd, &m_pddsZBuffer, NULL ) ) )
    {
...
//-----------------------------------------------------------------------------
// Name: ShowFrame()
// Desc: Show the frame on the primary surface, via a blt or a flip. Note
//       that without a back buffer, the rendered output already went
//       straight to the primary, so we do nothing in that case.
//       Note that sprite based apps will not call this function, but use
//       SetSpriteDisplayList() instead.
//-----------------------------------------------------------------------------
...
HRESULT CD3DFramework::RestoreSurfaces()
{
    ...
    // Note: Don't restore render surface, since it's just a duplicate ptr.
...
VOID CD3DFramework::Move( INT x, INT y )
{
    if( FALSE == m_bIsFullscreen )
    {
		INT nLeft   = x;
		INT nTop    = y;
		INT nRight  = x + m_dwRenderWidth;
		INT nBottom = y + m_dwRenderHeight;

        SetRect( &m_rcScreenRect, nLeft, nTop, nRight, nBottom );

		// If we have no backbuffer, then update viewport rect as well
		if( NULL == m_pddsBackBuffer )
			SetRect( &m_rcViewportRect, nLeft, nTop, nRight, nBottom );
    }
}
