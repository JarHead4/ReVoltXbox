...
//-----------------------------------------------------------------------------
extern DriverInfo* g_pCurrentDriver;     // The selected DD driver
...
// Name:  CopyToWideChar
//-----------------------------------------------------------------------------
VOID CopyToWideChar( WCHAR** pstrOut, LPTSTR strIn )
{
...
//-----------------------------------------------------------------------------
VOID AddDialogControl( WORD** pp, DWORD dwStyle, short x, short y,
...
//-----------------------------------------------------------------------------
VOID UpdateComboBoxesContent( HWND hDlg, DriverInfo** ppCurrentDriver, 
                              DeviceInfo** ppCurrentDevice, 
                              ModeInfo** ppCurrentMode, BOOL bWindowed )
{
    ...
    for( DriverInfo* pDriver = D3DEnum_GetFirstDriver(); pDriver; 
    ...
    for( DeviceInfo* pDevice = (*ppCurrentDriver)->pFirstDevice; pDevice; 
    ...
    for( ModeInfo* pMode = (*ppCurrentDevice)->pFirstMode; pMode; 
         pMode = pMode->pNext )
    {
...
//-----------------------------------------------------------------------------
// Name: DriverSelectProc()
// Desc: Windows message handling function for the driver select dialog
//-----------------------------------------------------------------------------
BOOL CALLBACK DriverSelectProc( HWND hDlg, UINT uiMsg, WPARAM wParam, 
                                LPARAM lParam )
{
    static DriverInfo *pOldDriver,  *pNewDriver;
    static DeviceInfo *pOldDevice,  *pNewDevice;
    static ModeInfo   *pOldMode,    *pNewMode;
        ...
        else if( CBN_SELENDOK == HIWORD(wParam) )
        {
            ...
            switch( LOWORD( wParam ) )
            {
                case IDC_DRIVER_COMBO:
                    ...
                    {
                        pNewDriver   = (DriverInfo*)pNewObject;
                ...
                case IDC_DEVICE_COMBO:
					...
					{
                        pNewDevice   = (DeviceInfo*)pNewObject;
                        pNewMode     = pNewDevice->pCurrentMode;
						bNewWindowed = pNewDevice->bWindowed;
					}
                    break;
